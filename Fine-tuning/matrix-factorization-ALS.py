...
# 原始评分矩阵 R (m个用户 × n个电影，有很多缺失值)
#             ↓ 分解
# 用户矩阵 U (m × k)  ×  物品矩阵 V (k × n)
#     用户→隐特征           隐特征→电影

# - U[i][f]：用户 i 对隐因子 f 的偏好程度
# - V[f][j]：电影 j 具有隐因子 f 的程度
# - R[i][j] ≈ U[i] · V[j]：两者点积 ≈ 原始评分

# 训练过程就是不断调整 U 和 V，让 U × V 在已知评分的位置上尽可能接近原始矩阵 R，误差（RMSE）在可接受范围内就算拟合成功。

# 之后对于缺失值（用户没打分的电影），直接用 U[i] · V[j] 的点积来预测，预测值高的就推荐给用户。

# 这就是 ALS 矩阵分解做推荐的全部核心思想。
