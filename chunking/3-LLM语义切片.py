...
def _semantic_chunking_fallback(text: str, max_chunk_size: int):
    """无 API 或 LLM 失败时，回退到「句子边界切片」模块。"""
    path = Path(__file__).resolve().parent / "2-句子边界切片.py"
    spec = importlib.util.spec_from_file_location("chunking_sentence_boundary", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("无法加载 2-句子边界切片.py")
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod.semantic_chunking(text, max_chunk_size)
...
def advanced_semantic_chunking_with_llm(text, max_chunk_size=512):
    ...
    if not api_key:
        ...
        return _semantic_chunking_fallback(text, max_chunk_size)
...
def test_chunking_methods():
    ...
    print("\n=== LLM语义切片测试 ===")
    try:
        chunks = advanced_semantic_chunking_with_llm(text, max_chunk_size=200)
