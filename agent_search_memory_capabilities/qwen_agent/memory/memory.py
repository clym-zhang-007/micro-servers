...
class Memory(Agent):
    ...
    def __init__(self,
            ...
            rag_cfg: The config for RAG. One example is:
              {
                ...
                'use_es': True,  # 是否使用Elasticsearch作为检索后端
                'use_es_vector': True  # 是否使用Elasticsearch向量检索
              }
        ...
        # 检查是否启用ES检索
        self.use_es: bool = self.cfg.get('use_es', False)
        # 检查是否使用ES向量检索
        self.use_es_vector: bool = self.cfg.get('use_es_vector', False)
        if not llm:
        ...
        if self.use_es_vector:
            # 使用ES向量检索工具
            super().__init__(function_list=[{
                'name': 'es_vector_retrieval',
                'max_ref_token': self.max_ref_token,
                'parser_page_size': self.parser_page_size,
                'rag_searchers': self.rag_searchers,
                'es_host': self.cfg.get('es_host', 'localhost'),
                'es_port': self.cfg.get('es_port', 9200),
                'es_username': self.cfg.get('es_username', 'elastic'),
                'es_password': self.cfg.get('es_password', os.getenv('ES_PASSWORD', '')),
                'vector_index_name': self.cfg.get('vector_index_name', 'qwen_agent_docs_vectors')
            }] + function_list,
                             llm=llm,
            ...
            super().__init__(function_list=[{
                ...
                'es_username': self.cfg.get('es_username', 'elastic'),
                'es_password': self.cfg.get('es_password', os.getenv('ES_PASSWORD', '')),
                'index_name': self.cfg.get('index_name', 'qwen_agent_docs')
            }] + function_list,
                             llm=llm,
                             system_message=system_message)
        else:
    ...
    def _run(self, messages: List[Message], lang: str = 'en', **kwargs) -> Iterator[List[Message]]:
        ...
        if not rag_files:
            ...
            # Determine which function to use based on configuration
            if self.use_es_vector:
                content = self.function_map['es_vector_retrieval'].call(
                    {
                        'query': query,
                        'files': rag_files
                    },
