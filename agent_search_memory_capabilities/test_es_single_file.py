# -*- coding: utf-8 -*-
"""ES 检索独立测试脚本 - 脱离 qwen-agent 包结构的独立验证

功能特性:
    - 独立运行：不依赖 qwen-agent 包，适合单独调试 ES 检索功能
    - 模拟依赖：内置 SimpleDocParserMock、DocParser、ElasticsearchSearcher 等类的简化版本
    - 文档解析 + 分块索引：使用 langchain 的 CharacterTextSplitter 进行文本切分
    - 检索验证：验证 ES 连接、索引创建、文档写入、搜索返回的完整链路

数据源:
    - Elasticsearch 实例：localhost:9200（索引 standalone_test_index）
    - 测试文件：docs/2-雇主责任险.txt

依赖:
    - elasticsearch
    - python-dotenv
    - langchain (CharacterTextSplitter)
    - transformers (AutoTokenizer，可选，失败时降级为简单分块)

运行方式:
    - 直接运行: python test_es_single_file.py
    - 前提: ES 服务已启动 + DASHSCOPE_API_KEY、ES_PASSWORD 环境变量已设置
"""
