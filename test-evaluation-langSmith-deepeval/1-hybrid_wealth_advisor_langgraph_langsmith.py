...
LANGSMITH_PROJECT = os.getenv("LANGCHAIN_PROJECT") or "wealth-advisor-hybrid-agent"
LANGSMITH_TRACING_RAW = os.getenv("LANGCHAIN_TRACING_V2", "")
LANGSMITH_ENABLED = LANGSMITH_TRACING_RAW.lower() == "true"

# 如果当前进程没有显式设置项目名，则注入默认项目名，避免 trace 落到默认项目里难以定位
os.environ.setdefault("LANGCHAIN_PROJECT", LANGSMITH_PROJECT)

print("LANGSMITH_ENABLED=", LANGSMITH_ENABLED)
print("LANGCHAIN_TRACING_V2 raw =", LANGSMITH_TRACING_RAW)
print("LANGCHAIN_PROJECT =", LANGSMITH_PROJECT)
print("LANGSMITH_API_KEY exists =", bool(LANGSMITH_API_KEY))
print("LANGSMITH_API_KEY prefix =", LANGSMITH_API_KEY[:8] if LANGSMITH_API_KEY else "")
print("LANGSMITH .env path =", CURRENT_DIR / ".env")

if LANGSMITH_ENABLED and not LANGSMITH_API_KEY:
    print("[WARN] LANGCHAIN_TRACING_V2 已开启，但未检测到 LANGSMITH_API_KEY，trace 将无法上报。")
elif LANGSMITH_ENABLED:
    print(f"[INFO] LangSmith tracing 已启用，项目名: {LANGSMITH_PROJECT}")
else:
