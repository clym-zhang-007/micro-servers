...
# 路由函数 - 根据当前阶段决定下一步
# 需要有add_conditional_edges（）才表示根据函数返回值动态决定下一跳去哪个节点
# decision 节点错误返回 current_phase = decision；未发生错误的话 current_phase = report（已经定位到下一个节点），所以router函数直接返回 state["current_phase"] 即可，无需if判断来定位到下一个节点
...
def create_research_agent_workflow() -> StateGraph:
    ...
    workflow.add_edge("report", END) # END 通过 langgraph.graph 引入，是 LangGraph 提供的特殊终止标记
    
    # 当 perception 节点执行完以后，不是固定走向某一个下游节点，而是先调用 router，再根据它返回的结果决定下一步去哪
    # 第三个参数对象是一个映射字典
