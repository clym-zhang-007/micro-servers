# -*- coding: utf-8 -*-
"""保险智能问答助手 v3 - 自定义 Gradio 界面（知乎直答风格）+ ES 向量检索

版本说明:
    在 v2 基础上引入自定义 Gradio 界面设计，参考知乎直答风格，支持多级检索降级策略。

功能特性:
    - 自定义 Gradio 界面：侧边栏知识库展示、推荐问题卡片、深度思考模式
    - ES 向量检索：优先使用 text-embedding 向量搜索，失败则降级到 ES 全文检索，再降级到默认检索
    - SQL 查询工具 (exc_sql)：连接 MySQL 数据库执行查询，自动生成柱状图
    - 多轮对话支持：聊天历史记录展示
    - 知识源切换：保险文档库 / 公开知识 / SQL 数据库
...
依赖:
    - qwen_agent, dashscope, gradio, python-dotenv
    - pandas, sqlalchemy, mysql-connector-python
    - matplotlib, numpy

运行方式:
    - 直接运行: python insurance_qa_agent-3.py (启动自定义 Gradio 界面)
    - 环境变量: 需设置 DASHSCOPE_API_KEY、ES_PASSWORD、DB_* 等
"""
import os
