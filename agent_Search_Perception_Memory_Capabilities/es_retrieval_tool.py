# -*- coding: utf-8 -*-
"""基于 Elasticsearch 的 RAG 检索工具

功能特性:
    - 自动文件索引：首次查询时自动读取文件、分块并写入 ES
    - 多格式解析：支持 PDF (pypdf)、TXT 格式
    - 智能分块：按 chunk_size（默认 500）切分长文本
    - 全文检索：基于 IK 分词器（若有）或标准分词器的 match 查询
    - 增量更新：已索引文件自动跳过，避免重复写入
    - Bulk API 批量写入：高效索引大量文档块

数据源:
    - Elasticsearch 实例：localhost:9200（索引 qwen_agent_rag_index）
    - 支持的文件格式：PDF、TXT

典型使用场景:
    - 作为 qwen-agent Memory 的 ES 后端替代默认检索
    - 保险条款、产品文档等知识库的全文检索
...
使用方式:
    - 将此工具注册到 qwen-agent 的 function_list 中
    - 通过 rag_cfg 配置 ES 连接参数和索引名称
"""
import os
...
class ElasticsearchRetrievalTool(BaseTool):
    ...
    def __init__(self, cfg: dict = None):
        ...
        self.es_password = self.cfg.get('password', os.getenv('ES_PASSWORD', '')) # 从环境变量读取
