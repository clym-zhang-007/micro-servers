# -*- coding: utf-8 -*-
"""保险智能问答助手 v4 - 基于 Qwen Agent + ES 向量检索 + WebUI

版本说明:
    基于 Qwen Agent 原生 WebUI，支持 ES 向量检索，带多级降级策略。

功能特性:
    - ES 向量检索：使用 text-embedding 向量进行语义搜索
    - 多级降级策略：向量检索 → ES 全文检索 → 默认检索
    - SQL 查询工具 (exc_sql)：连接 MySQL 数据库执行查询，自动生成柱状图
    - 使用 Qwen Agent 原生 WebUI 界面
...
运行方式:
    - 直接运行: python insurance_qa_agent-4.py (默认启动 WebUI)
