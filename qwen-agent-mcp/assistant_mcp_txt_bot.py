...
def init_agent_service():
    ...
    # 系统角色设定
    system = ('你是一个文本分析助手，你具有计算文本字符数、单词数、行数等能力。'
             '你可以帮助用户分析文本的基本统计信息。'
    ...
    tools = [{
        "mcpServers": {
            "txt-counter": {
                # 启动 MCP 服务时，用当前正在运行这个脚本的 Python 解释器去启动它
                "command": os.path.abspath(os.sys.executable),
                # 启动这个 MCP 服务时，会执行txt_counter.py
                # txt_counter.py：服务端 / MCP 侧，负责提供工具；在代码中通过FastMCP启动了一个MCP server
                # assistant_mcp_txt_bot.py：客户端 / Agent 侧，负责接工具
                "args": [os.path.join(os.path.dirname(__file__), "txt_counter.py")]
            }
        }
    }]
